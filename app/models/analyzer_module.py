...
class ChizhevskAnalyzer:
    ...
    def comprehensive_correlation_analysis(
        self, solar_data: pd.DataFrame, biological_data: pd.DataFrame, biological_events: List[Dict]
    ) -> Dict[str, Any]:
        """
    ...
    def _create_biological_event_series(
        self, events: List[Dict], date_index: pd.DatetimeIndex
    ) -> pd.Series:
        ...
        for event in events:
            try:
                ...
                end_date = pd.Timestamp(
                    event.get("end_date") or event.get("end_year", start_date.year + 1)
                )
                ...
                if event_mask.sum() > 0:
                    ...
                    if duration_months > 1:
                        beta_curve = stats.beta.pdf(np.linspace(0, 1, duration_months), 2, 5)
    ...
    def _multiple_correlation_analysis(
        self, solar_ts: pd.Series, bio_ts: pd.Series
        ...
        correlations["pearson"] = CorrelationResult(
            ...
            p_value=pearson_p,
            confidence_interval=self._correlation_confidence_interval(pearson_r, len(solar_ts)),
        ...
        correlations["spearman"] = CorrelationResult(
            ...
            p_value=spearman_p,
            confidence_interval=self._correlation_confidence_interval(spearman_r, len(solar_ts)),
            sample_size=len(solar_ts),
            degrees_of_freedom=len(solar_ts) - 2,
        ...
        correlations["kendall"] = CorrelationResult(
            ...
            p_value=kendall_p,
            confidence_interval=self._correlation_confidence_interval(kendall_r, len(solar_ts)),
            sample_size=len(solar_ts),
            degrees_of_freedom=len(solar_ts) - 2,
        ...
        for method in ["pearson", "spearman", "kendall"]:
            bootstrap_ci = self._bootstrap_correlation_ci(solar_ts.values, bio_ts.values, method)
    ...
    def _advanced_spectral_analysis(self, solar_ts: pd.Series, bio_ts: pd.Series) -> Dict[str, Any]:
        ...
        results["power_spectral_density"] = self._power_spectral_analysis(solar_interp, bio_interp)

        # 2. Análisis de coherencia espectral
        results["spectral_coherence"] = self._spectral_coherence_analysis(solar_interp, bio_interp)
    ...
    def _prepare_for_spectral_analysis(
        self, solar_ts: pd.Series, bio_ts: pd.Series
    ...
    def _power_spectral_analysis(
        self, solar_data: np.ndarray, bio_data: np.ndarray
    ) -> Dict[str, Any]:
        """Análisis de densidad espectral de potencia"""

        # Calcular PSD usando método de Welch
        if self.config.spectral_method == "welch":
            f_solar, psd_solar = signal.welch(solar_data, nperseg=min(len(solar_data) // 4, 256))
            f_bio, psd_bio = signal.welch(bio_data, nperseg=min(len(bio_data) // 4, 256))
        else:
    ...
    def _spectral_coherence_analysis(
        self, solar_data: np.ndarray, bio_data: np.ndarray
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            f, coherence = signal.coherence(
                solar_data, bio_data, nperseg=min(len(solar_data) // 4, 256)
            )
    ...
    def _detect_solar_harmonics(self, solar_data: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            "closest_to_11_year": (
                min(top_periods, key=lambda x: abs(x - 11)) if top_periods else None
            ),
        }
    ...
    def _interpret_lag(self, lag_months: int) -> str:
        ...
        if lag_months == 0:
            ...
            return (
                f"Eventos biológicos tienden a preceder actividad solar por {abs(lag_months)} meses"
            )
    ...
    def _causality_analysis(self, solar_ts: pd.Series, bio_ts: pd.Series) -> Dict[str, Any]:
        ...
        try:
            data_df = pd.DataFrame({"solar": solar_ts, "biological": bio_ts}).dropna()
            ...
            try:
                ...
                granger_solar_to_bio = grangercausalitytests(
                    data_df[["biological", "solar"]], maxlag=max_lag, verbose=False
                )

                # Biological -> Solar
                granger_bio_to_solar = grangercausalitytests(
                    data_df[["solar", "biological"]], maxlag=max_lag, verbose=False
                )
    ...
    def _statistical_validation(
        self, solar_ts: pd.Series, bio_ts: pd.Series, correlations: Dict[str, CorrelationResult]
    ) -> Dict[str, Any]:
    ...
    def _temporal_cross_validation(self, solar_ts: pd.Series, bio_ts: pd.Series) -> Dict[str, Any]:
    ...
    def _assess_chizhevsky_theories(
        self,
        solar_ts: pd.Series,
        bio_ts: pd.Series,
        events_ts: pd.Series,
        ...
        if main_correlation:
            if (
                main_correlation.statistical_significance
                and abs(main_correlation.correlation_coefficient) > 0.3
            ):
        ...
        # Resumen general
        supported_theories = len(
            [v for v in assessment["theory_validations"].values() if v.get("supported")]
        )
        total_theories = len(assessment["theory_validations"])

        if supported_theories == 0:
            assessment["overall_assessment"] = (
                "Las teorías de Chizhevsky no reciben apoyo estadístico significativo en estos datos"
            )
        elif supported_theories == total_theories:
            assessment["overall_assessment"] = (
                "Las teorías fundamentales de Chizhevsky reciben fuerte apoyo estadístico"
            )
        else:
            assessment["overall_assessment"] = (
                f"Apoyo parcial a las teorías de Chizhevsky ({supported_theories}/{total_theories} validaciones)"
            )

        return assessment

    def _analyze_by_solar_cycle_phase(
        self, solar_ts: pd.Series, bio_ts: pd.Series
    ) -> Dict[str, Any]:
...
def create_correlation_report(analysis_results: Dict[str, Any]) -> str:
    ...
    for method, result in correlations.items():
        if isinstance(result, dict) and "correlation_coefficient" in result:
            report.append(
                f"- {method.capitalize()}: r={result['correlation_coefficient']:.3f}, "
...
async def test_analyzer():
    ...
    solar_data = pd.DataFrame(
        {"date": dates, "sunspot_number": np.maximum(0, solar_cycle + solar_noise)}
    )

    # Datos biológicos sintéticos correlacionados
    bio_base = solar_cycle * 0.3 + np.random.normal(0, 10, len(dates))
    bio_data = pd.DataFrame({"date": dates, "biological_index": bio_base})
    ...
    results = analyzer.comprehensive_correlation_analysis(solar_data, bio_data, events)
