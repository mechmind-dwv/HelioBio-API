...
class SolarEventPrediction(BaseModel):
    ...
    probability: float = Field(
        ..., ge=0.0, le=1.0, description="Probabilidad del evento (0.0 a 1.0)."
    )
    prediction_time_utc: str = Field(..., description="Tiempo UTC de la predicción.")
    confidence_score: float = Field(
        ..., ge=0.0, le=1.0, description="Puntuación de confianza del modelo."
    )
...
class BiologicalImpactPrediction(BaseModel):
    ...
    predicted_severity: int = Field(
        ..., ge=1, le=10, description="Severidad pronosticada (1 a 10)."
    )
    explanation: str
...
@router.get(
    "/predictions/solar_event",
    response_model=SolarEventPrediction,
    summary="Predecir un evento solar futuro",
)
def predict_solar_event(
    date: str = Query(
        ...,
        example="2025-09-07",
        description="Fecha para la que se desea la predicción (formato YYYY-MM-DD).",
    )
...
@router.get(
    "/predictions/biological_impact",
    response_model=BiologicalImpactPrediction,
    summary="Predecir el impacto biológico de un evento solar",
)
def predict_biological_impact(
    solar_event_id: str = Query(
        ..., example="X1-20250907", description="ID del evento solar a analizar."
    )
