...
def subscribe_to_alerts(contact: AlertContact) -> Dict[str, str]:
    ...
    print(
        f"Nuevo contacto suscrito: ID {contact.contact_id}, Tipo: {contact.contact_type}, Valor: {contact.value}"
    )
...
@router.get(
    "/alerts/latest",
    response_model=List[GeneratedAlert],
    summary="Obtener las últimas alertas generadas",
)
